	...
	)
{
	...
	if ((temp = yj1_get_bits(src, bitptr, 2)) != 0)
	{
		if (yj1_get_bits(src, bitptr, 1))
			...
			return SDL_Swap16LE(header->LZSSRepeatTable[temp]);
	}
	else
		return SDL_Swap16LE(header->LZSSRepeatTable[0]);
}
	...
	)
{
	...
	if (Source == NULL)
		return -1;
	if (SDL_Swap32LE(hdr->Signature) != 0x315f4a59)
		return -1;
	if (SDL_Swap32LE(hdr->UncompressedLength) > (unsigned int)DestSize)
		return -1;
	...
	for (i = 0; i < SDL_Swap16LE(hdr->BlockCount); i++)
	{
		unsigned int bitptr;
		PYJ_1_BLOCKHEADER header;
		...
		src += 4;
		if (!SDL_Swap16LE(header->CompressedLength))
		{
			unsigned short hul = SDL_Swap16LE(header->UncompressedLength);
			while (hul--)
			{
				*dest++ = *src++;
			}
		...
		src = ((unsigned char *)header) + SDL_Swap16LE(header->CompressedLength);
	}
	free(root);

	return SDL_Swap32LE(hdr->UncompressedLength);
}
