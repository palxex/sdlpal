 ...
 *   You can just #include <SDL3/SDL_stbimage.h> (without the #define) in other source
 *   files to use it there. (See also below this comment for an usage example)
 *   This header implicitly #includes <SDL3/SDL.h> and "stb_image.h".
 *
 ...
 *   CRT on windows and with SDL you're better off using SDL_IOStream, incl. SDL_IOFromFile()
...
// if you really think you need <SDL3/SDL.h> here instead.. feel free to change it,
...
// If freeWithSurface is true, SDL_DestroySurface() will free the pixelData
...
SDL_STBIMG_DEF bool STBIMG_stbi_callback_from_RW(SDL_IOStream* src, STBIMG_stbio_RWops* out);
  ...
  Sint64 origSrcPosition = SDL_TellIO(src);
  int w, h, fmt;
  ...
  if(SDL_SeekIO(src, origSrcPosition, SDL_IO_SEEK_SET) < 0)
  {
...
// - without using SDL_SeekIO(), for streams that don't support or are slow
...
// make stb_image use SDL_malloc etc, so SDL_DestroySurface() can SDL_free()
...
static SDL_Surface* STBIMG__CreateSurfaceImpl(STBIMG__image img, int freeWithSurface)
{
	...
	if(freeWithSurface)
	{
		...
		// clear the SDL_SURFACE_PREALLOCATED flag, so SDL_DestroySurface() free()s the data passed from img.data
...
SDL_STBIMG_DEF SDL_Surface* STBIMG_Load_RW_noSeek(SDL_IOStream* src, int freesrc)
{
	...
	{
		SDL_SetError("STBIMG_Load_RW_noSeek(): SDL_GetIOSize(src) returned 0 => empty file/stream?!");
		goto end;
	}
	else if(fileSize > 0x7FFFFFFF)
	{
		...
		SDL_SetError("STBIMG_Load_RW_noSeek(): SDL_GetIOSize(src) too big (> 2GB)!");
		goto end;
	}
