	...
	)
{
	...
	if (iNum > 0)
	{
		if (gConfig.fIsWIN95)
		{
			player->pSong = Timidity_LoadSong(SDL_RWFromFile(UTIL_GetFullPathName(PAL_BUFFER_SIZE_ARGS(0), gConfig.pszGamePath, PAL_va(1, "Musics%s%.3d.mid", PAL_NATIVE_PATH_SEPARATOR, iNum)), "rb"), AUDIO_GetDeviceSpec());
		}
